  ...
  return (
    <div className="space-y-5">
      ...
      <div className="card-surface p-6">
        ...
        <form onSubmit={(e) => { e.preventDefault(); const fd = new FormData(e.currentTarget); const next = updateVendorStoreProfile({ name: String(fd.get("name") ?? ""), description: String(fd.get("description") ?? ""), city: String(fd.get("city") ?? ""), phone: String(fd.get("phone") ?? "") }); setProfile(next); toast("تغییرات فروشگاه در همین دمو ذخیره شد"); }} className="grid gap-4 sm:grid-cols-2">
