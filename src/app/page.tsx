// ============================================================
// Home page — SERVER component (Task 33 refactor).
//
// Was: one 470-line "use client" page. Now: a server component
// that ships real HTML for every section (categories, styles,
// collections, trends, magazine, stores) and mounts exactly ONE
// client island — the cinematic intro hero (see `HomeHero`).
//
// Server-side wins:
//   • `export const metadata` (impossible in client pages) —
//     canonical + dedicated SEO title/description for `/`.
//   • product/store arrays no longer ship in the client bundle
//     just to render two counts — they are read at build time.
// ============================================================
import type { Metadata } from "next";
import Link from "next/link";
import { Search, Sparkles, ArrowLeft, Play, Wand2, Lightbulb, Store, BadgeCheck, ShieldCheck, HeartHandshake, Truck } from "lucide-react";
...
export const metadata: Metadata = {
  title: {
    absolute:
      "Homeino | بازارگاه خانه و دکوراسیون — الهام، استودیو طراحی هوشمند و خرید مطمئن",
  },
  description:
    "خانه‌ای که شبیه توست — با Homeino الهام بگیر، فضایت را با هومینو استودیو طراحی کن و از فروشگاه‌های معتبر با ضمانت خرید کن؛ راهنمای سبک‌ها، ترندهای روز و مجله‌ی دکوراسیون به فارسی.",
        ...
        <Container>
          ...
          <div className="hide-scrollbar scroll-fade -mx-4 flex snap-x gap-3 overflow-x-auto px-4 pb-4 sm:-mx-8 sm:px-8 lg:mx-0 lg:grid lg:grid-cols-5 lg:overflow-visible lg:px-0">
            ...
            <Link href="/styles" aria-label="مشاهده همه سبک‌ها" className="group card-surface card-interactive flex w-[72vw] max-w-64 shrink-0 snap-start flex-col items-center justify-center gap-4 bg-gradient-to-br from-ivory to-cream p-6 text-center lg:w-auto" style={{ aspectRatio: "4 / 5" }}>
              ...
              <span className="text-2xs leading-5 text-ink-muted">راهنمای کامل {toFa(styles.length)} سبک؛ پالت رنگ، متریال و محصولات هر سبک</span>
            </Link>
          </div>
        </Container>
      </section>
