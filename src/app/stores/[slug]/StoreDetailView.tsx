"use client";
// ============================================================
// StoreDetailView — نمای صفحهٔ فروشگاه (Task 60)
//
// داده از سرور می‌آید (page.tsx سرور). فقط برای فروشگاه نمونهٔ دمو،
// ویرایش‌های localStorage پنل فروشنده (vendorSession) روی همین نما
// اعمال می‌شود — فروشگاه‌های واقعیِ DB همیشه دادهٔ واقعی نشان می‌دهند.
// ============================================================
...
import type { StorePageData } from "@/services/storefront";
import { StoreMessagePanel } from "./StoreMessagePanel";

export function StoreDetailView({ data }: { data: StorePageData }) {
  // re-render once the persisted vendor session lands (post-hydration) so the
  // DEMO vendor's saved profile/products show up — real DB stores are untouched
  const vsVersion = useVendorSessionVersion();
  void vsVersion;

  const isDemo = data.source === "demo";
  const store = isDemo ? resolvePublicStore(data.store) : data.store;
  const products = isDemo ? allStoreProductsPublic(data.store.id) : data.products;
    ...
    <Container className="py-8">
      ...
      {profile && (
        <div className="mt-5 flex flex-wrap gap-2">
          ...
          {profile.responseTime && <span className="flex items-center gap-1.5 rounded-full border border-clay/50 bg-ivory-2 px-3 py-1.5 text-xs text-ink"><Timer size={13} className="text-terracotta-deep" /> پاسخ‌گویی {profile.responseTime}</span>}
          {profile.shippingCoverage && <span className="flex items-center gap-1.5 rounded-full border border-clay/50 bg-ivory-2 px-3 py-1.5 text-xs text-ink"><Truck size={13} className="text-terracotta-deep" /> {profile.shippingCoverage}</span>}
      ...
      {/* Trust, policies & store reviews — DB store data with honest demo fallback */}
      <div className="mt-6 grid gap-4 lg:grid-cols-[1fr_380px]">
        <div className="card-surface p-5">
          ...
          <div className="space-y-3 text-sm leading-7">
            {store.shippingPolicy && <div><span className="font-bold text-ink">سیاست ارسال: </span><span className="text-ink-muted">{store.shippingPolicy}</span></div>}
            {store.returnPolicy && <div><span className="font-bold text-ink">سیاست بازگشت: </span><span className="text-ink-muted">{store.returnPolicy}</span></div>}
          </div>
        </div>
        <div className="card-surface p-5">
      ...
      {/* گفتگوی مستقیم با فروشگاه (Task 60) — پیام مشتری → اعلان + SMS فروشنده */}
      <StoreMessagePanel slug={store.slug} source={data.source} className="mt-8" />
    </Container>
  );
}
