        ...
        <div className="fixed inset-0 z-[110] bg-ink/50 backdrop-blur-sm lg:hidden" onClick={() => setMobileNav(false)}>
          <div className="absolute right-0 top-0 h-full w-72 bg-cream p-5 shadow-[var(--shadow-lift)]" onClick={(e) => e.stopPropagation()}>
            ...
            <nav className="space-y-1">
              {[["خانه", "/"], ["همه محصولات", "/products"], ["دسته‌بندی‌ها", "/category/furniture"], ["فروشگاه‌ها", "/stores"], ["الهام", "/inspiration"], ["سبک‌ها", "/styles"], ["AI استودیو", "/ai"], ["مقایسه", "/compare"], ["علاقه‌مندی", "/wishlist"], ["سبد خرید", "/cart"], ["حساب کاربری", "/account"], ["پنل فروشنده", "/vendor"], ["پنل مدیریت", "/admin"]].map(([label, href]) => (
