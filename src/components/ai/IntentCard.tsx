...
export function IntentCard({
  ...
  if (!analysis) {
    if (understanding) {
      return (
        <div className="flex items-center gap-2 rounded-xl border border-clay/40 bg-cream px-3.5 py-2.5 text-[11px] text-ink-muted">
    ...
    <div className={cn(
      ...
      <div className="mb-2.5 flex flex-wrap items-center justify-between gap-2">
        <div className="flex items-center gap-1.5 text-[11px] font-bold text-ink">
          ...
          AI این‌طور درک کرد:
          <span className={cn("rounded-full px-2 py-0.5 text-[10px] font-bold", isFull ? "bg-terracotta/15 text-terracotta-deep" : "bg-sage/15 text-sage-deep")}>
        ...
        <div className="flex items-center gap-1.5" title="میزان اطمینان AI از درک درخواست">
          ...
          <span className="text-[10px] font-bold tabular-nums text-ink-muted">٪{toFa(confidencePct)}</span>
        </div>
      </div>

      {/* targets — editable */}
      <div className="flex flex-wrap items-center gap-1">
        <span className="text-[10px] font-bold text-ink-muted">تغییر می‌کند:</span>
        {isFull ? (
          <span className="rounded-md bg-terracotta/15 px-2 py-0.5 text-[10px] font-bold text-terracotta-deep">کل فضا (با اجازه‌ی تو)</span>
        ) : analysis.target.length === 0 ? (
          <span className="text-[10px] text-ink-muted">هنوز چیزی مشخص نشده</span>
        ) : (
          <>
            {analysis.target.map((el) => (
              <button
                ...
                className="group flex items-center gap-1 rounded-md border border-terracotta/40 bg-terracotta/10 px-2 py-0.5 text-[10px] font-bold text-terracotta-deep transition hover:bg-terracotta/20"
            ...
            {onToggleTarget && (
              <span className="text-[9px] text-ink-muted">برای حذف، روی مورد کلیک کن</span>
            )}
          </>
        )}
      </div>
      ...
      {!isFull && (
        <div className="mt-1.5 flex flex-wrap items-center gap-1">
          <span className="text-[10px] font-bold text-ink-muted">حفظ می‌شود:</span>
          {analysis.preservedElements.slice(0, 7).map((el) => (
            <span key={el} className="flex items-center gap-0.5 rounded-md bg-ivory-2 px-1.5 py-0.5 text-[10px] text-ink-muted">
              <Lock size={8} /> {ELEMENT_LABELS[el]}
            </span>
          ))}
          {analysis.preservedElements.length > 7 && (
            <span className="text-[10px] text-ink-muted">و {toFa(analysis.preservedElements.length - 7)} مورد دیگر</span>
          )}
        </div>
      )}
      ...
      {(analysis.colors?.length ?? 0) > 0 && (
        <div className="mt-1.5 flex flex-wrap items-center gap-1">
          <span className="text-[10px] font-bold text-ink-muted">رنگ‌ها:</span>
          {analysis.colors!.map((c) => (
            <span key={c} className="rounded-md bg-ivory-2 px-1.5 py-0.5 text-[10px] text-ink-muted">{c}</span>
          ))}
        </div>
      )}

      {analysis.changes.length > 0 && (
        <p className="mt-1.5 text-[10px] leading-5 text-ink-muted">«{analysis.changes.join("؛ ")}»</p>
      )}

      {lowConfidence && (
        <p className="mt-2 flex items-start gap-1 text-[10px] leading-5 text-gold">
          <AlertTriangle size={11} className="mt-0.5 shrink-0" />
          {analysis.note ?? "درخواست دقیق نیست — مثلاً بنویس: «مبل را عوض کن» یا «رنگ دیوار را کرم کن»."}
        </p>
      )}
      {!lowConfidence && analysis.note && (
        <p className="mt-1.5 text-[10px] leading-5 text-ink-muted">{analysis.note}</p>
      )}
    </div>
  );
}
      ...
      {ALL_ELEMENTS.map((el) => {
        ...
        return (
          <button
            ...
            onClick={() => toggle(el)}
            className={cn(
              "flex items-center gap-1 rounded-lg border px-2 py-1 text-[10px] font-bold transition disabled:opacity-40",
