...
import {
  ...
  getProductBySku as mockBySku, skuOf as mockSkuOf,
...
export interface ProductsRepository {
  ...
  byId(id: string): Promise<Product | undefined>; bySku(sku: string): Promise<Product | undefined>;
...
function withSku(product?: Product): Product | undefined {
  if (!product) return undefined;
  return { ...product, sku: product.sku ?? mockSkuOf(product) };
}

export const productsRepository: ProductsRepository = {
  list: async () => hasDatabase() ? remoteList() : mockProducts.map((p) => withSku(p)!),
  bySlug: async (slug) => {
    if (!hasDatabase()) return withSku(mockBySlug(slug));
  ...
  byId: async (id) => hasDatabase() ? (await remoteList()).find(p => p.id === id) : withSku(mockById(id)),
  bySku: async (sku) => {
    const local = mockBySku(sku);
    if (!hasDatabase()) return local;
    try {
      const { getProductBySku } = await import("@/services/catalogService");
      return toDomain(await getProductBySku(sku));
    } catch {
      return local;
    }
  },
  byCategory: async (slug) => hasDatabase() ? remoteList({ categorySlug: slug }) : mockByCategory(slug).map((p) => withSku(p)!),
  byStyle: async (slug) => hasDatabase() ? remoteList({ styleSlug: slug }) : mockByStyle(slug).map((p) => withSku(p)!),
  similar: async (id, take = 4) => hasDatabase() ? (await remoteList()).filter(p => p.id !== id).slice(0, take) : mockSimilar(id, take).map((p) => withSku(p)!),
  trending: async (take = 12) => hasDatabase() ? remoteList({ sort: "popular", limit: take }) : mockTrending.slice(0, take).map((p) => withSku(p)!),
