...
export const BUILTIN_TOOLS: ToolRecord[] = [
  ...
  { key: "getVendor", name: "دریافت فروشنده", description: "خواندن پروفایل عمومی فروشگاه/فروشنده با id یا slug", category: "vendor", requiredPermission: "READ_VENDORS", requiresApproval: false, isDestructive: false, inputSchema: { vendorId: "string?", vendorSlug: "string?" }, isActive: true, isBuiltin: true },
  { key: "getStore", name: "دریافت فروشگاه", description: "خواندن نمای عمومی فروشگاه برای مشتری", category: "vendor", requiredPermission: "READ_VENDORS", requiresApproval: false, isDestructive: false, inputSchema: { storeId: "string?", storeSlug: "string?" }, isActive: true, isBuiltin: true },
  { key: "runWorkflow", name: "اجرای ورک‌فلو", description: "صف‌کردن یک اجرای ورک‌فلوی ثبت‌شده — ورک‌فلو مجوزهای داخلی خودش را دارد", category: "automation", requiredPermission: "WRITE_TASKS", requiresApproval: false, isDestructive: false, inputSchema: { workflowKey: "string", input: "object?", triggerKind: "string?" }, isActive: true, isBuiltin: true },
...
export const BUILTIN_AGENTS: BuiltinAgent[] = [
  ...
  },
  {
    isBuiltin: true,
    key: "vendor-agent",
    name: "ایجنت فروشندگان",
    description:
      "گزارش عملکرد هر فروشگاه: پروفایل واقعی فروشنده، شمار محصول، میانگین قیمت، سلامت موجودی و محصولات کم‌موجود — خروجی فقط گزارش و وظیفه؛ هیچ تغییری در قیمت/موجودی بدون تأیید انسانی.",
    type: "analyzer",
    status: "active",
    runtime: "local",
    handler: "vendor",
    ...
    config: { lowStockThreshold: 3 },
    permissions: ["READ_VENDORS", "READ_PRODUCTS", "READ_INVENTORY", "WRITE_TASKS", "SEND_NOTIFICATION"] as AgentPermissionKey[],
    tools: ["getVendor", "getStore", "searchProducts", "getInventory", "createTask", "sendNotification"],
  },
  {
    isBuiltin: true,
    key: "marketing-agent",
    name: "ایجنت بازاریابی",
    description:
      "بخش‌بندی مشتریان از رویدادهای رفتاری واقعی و ساخت پیش‌نویس کمپین. ارسال کمپین یا تغییر قیمت فقط با تأیید انسانی — خودش هرگز مستقیم چیزی ارسال نمی‌کند.",
    type: "analyzer",
    status: "active",
    runtime: "local",
    handler: "marketing",
    ...
    config: { sinceDays: 30 },
    permissions: ["READ_ANALYTICS", "READ_CUSTOMERS", "WRITE_TASKS", "REQUEST_APPROVAL"] as AgentPermissionKey[],
    tools: ["getCustomerEvents", "getCustomerPreferences", "createTask", "requestApproval"],
  },
  {
    isBuiltin: true,
    ...
    name: "ایجنت توسعه",
    description:
      "تبدیل تسک توسعه به پلن پیاده‌سازی ساختاریافته (فایل‌ها، مراحل، تست‌ها، ریسک‌ها) برای بازبینی انسانی. فقط پلن: بدون دسترسی نوشتن به کد و بدون مسیر deploy.",
    type: "assistant",
    status: "draft",
    runtime: "local",
    handler: "development",
    maxRetries: 1,
    timeoutMs: 45000,
    config: { scope: "general" },
    permissions: ["CALL_LLM", "WRITE_TASKS", "REQUEST_APPROVAL"] as AgentPermissionKey[],
    tools: ["llmComplete", "createTask", "requestApproval"],
  },
  // ------------------------------------------------------------
